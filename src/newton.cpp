...
namespace NOL
{
    ...
    Matrix QuasiNewton::CorrectHk(Matrix Hk, Vector sk, Vector yk)
    {
        ...
        {
            // DFP
            return Hk + (sk * sk.transpose()) / (sk.transpose() * yk) - (Hk * yk * yk.transpose() * Hk) / (yk.transpose() * Hk * yk);
        }
        else if (_options->_quasi_newton_type == QuasiNewtonType::BFGS)
        {
            // BDGS
            /*return (
                Hk+(Matrix::Identity(sk.size(),sk.size())+(yk.transpose()*Hk*yk)/(yk.transpose()*sk))*((sk * sk.transpose())/(yk.transpose()*sk))
